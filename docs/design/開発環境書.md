...
|---|---|---|
| 1.0 | 2026-09-13 | 初版作成 |
| 1.1 | 2026-09-21 | Claude Code on the webからGitHubへの接続方法を、Claude GitHub App方式に更新（PATを用いた旧運用の記載を削除） |
| 1.2 | 2026-09-21 | GitHubリポジトリの公開範囲について、Private固定の記載をプロジェクトの運用方針に応じた設定へ修正 |

## 0. 本書について

本書は、本プロジェクト（BlueCom開発）の開発者・試験担当者向けに、開発環境の構築方法・ビルドからマージまでの流れ・E2E試験環境の構築方法をまとめた運用手順書である。
...
| `docs/rule/コーディング規約.md` | ソースコードの命名規則・アーキテクチャ方針・ファイル構成ルール |
| `docs/design/操作説明書.md` | エンドユーザー向けの操作手順 |
| `CLAUDE/プロジェクト概要.md` | `CLAUDE.md`が参照する、本プロジェクト固有の情報（読むべきドキュメント一覧・アーキテクチャ・ビルド環境の制約等） |
| `docs/design/開発環境書.md` | 本書。開発環境構築・ビルド・E2E試験環境・リポジトリ管理の手順 |
| `CLAUDE/設計検査仕様書作成.md` | 設計検査仕様書（`docs/Test specification/BlueCom設計検査仕様書.md`）の作成・修正時にAIが参照する作成ルール |
| `CLAUDE/試験中.md` | 設計検査仕様書に基づく試験の実施中にAIが参照する記録・運用ルール |
...
2. **GitHubでリポジトリを作成する**
   - リポジトリの公開範囲（Public／Private）は、プロジェクトの運用方針に応じて設定する
3. **Claude Code on the webからGitHubリポジトリを操作するための接続を設定する**
   1. Claude.aiでGitHubとの接続を有効にする。
   2. GitHubアカウントにClaude GitHub Appをインストールする。
   3. GitHub Appのインストール設定で、Repository accessを「Only select repositories」にする。
   4. Repository accessの選択対象に、Claude GitHub Appからのアクセスを許可する開発対象リポジトリを追加する。
   5. GitHubの`Settings → Applications → Installed GitHub Apps → Claude → Configure`で、Claude GitHub Appから開発対象リポジトリへのアクセスが許可されていることを確認する。
   6. Claude Code on the webから対象リポジトリを開き、コードの取得（clone）およびpushが可能であることを確認する。

   > Claude Code on the webからGitHubを操作するための認証にはClaude GitHub Appを使用し、Personal Access Token（PAT）をClaude Codeのチャットやセッションへ入力しない。

## 3. ビルド

通常の開発サイクルは、以下の手順で行う。

1. **Claude Code on the webでコードを生成し、コミット・pushする**
2. **Working CopyでiPadにリポジトリをクローンし、対象のブランチに切り替える**
   - 2回目以降、既にクローン済みのブランチを最新化する場合は、Fetchだけで済ませず、必ずPullまで行う（Fetchはリモートの情報を取得するだけで、ローカルの作業ツリーは更新されない。Fetchのみで次の手順に進むと、Swift Playgroundsが古いコードのままビルドしてしまう）
3. **Swift Playgroundsでビルドを行う**
   - エラーまたは警告が発生した場合は、画面のキャプチャを撮り、Claudeへ提示して修正案を提案させる
   - 修正は承認後にのみ実装・修正させる（無断で修正させない。詳細な運用方針はCLAUDE.md 1.4「CLAUDEの意思決定権」および1.7「調査範囲と変更範囲を区別する」を参照）
...
5. **プロンプトでレビューを行う**
   ...
   - レビューは`CLAUDE.md`に定めた役割・観点に基づいて行わせる
...
E2E試験は、TestFlight経由で実機2台にアプリを配布し、`docs/design/操作説明書.md`に沿って実施する。以下、TestFlightで配布可能にするための手順を、App Store Connectでの新規App追加から記載する。
...
- `docs/design/操作説明書.md`の手順に沿って、実機2台で実際に操作しながら確認する

...
| 1.8 | 2026-08-23 | 6.4節に、Central役の切断完了がCore Bluetoothから非同期に通知される点と`BluetoothManager.isDisconnecting`の扱いを追記（実装済みだが未反映だった内容の反映）。4.5節・6.5節・6.6節に、データ受信側からの応答（送信種別`0x92`）を追加。6.5節は応答検出（10秒）を正常/異常終了の判定条件に、6.6節はチェックOK時の応答送信を追記（実機試験で判明した、受信側が受信中でない場合に送信側が誤って成功と判定される不具合への対応。要件定義書8章・9章と対応） |

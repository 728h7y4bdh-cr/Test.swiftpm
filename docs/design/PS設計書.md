...
本書は BlueCom のプログラム内部仕様を定義する。内部パラメータ、状態遷移、Bluetooth通信フォーマット、Core Bluetoothを用いた通信処理の詳細フローを記載する。画面仕様は「SS設計書」、画面レイアウトは「UI設計書」を参照すること。
...
| 4 | GATT通信方向 | Central→Peripheralへの送信はWrite特性、Peripheral→Centralへの送信はNotify特性を用いる（片方向2特性構成） | Central→Peripheralへの送信（通信開始要求・データ送信）とPeripheral→Centralへの送信（通信開始応答・データ受信応答）は、それぞれ送信方向が固定されているため、方向ごとに1特性ずつの2特性で全シナリオを表現できる |
...
| データ受信応答（通信種別`0x02`、送信種別`0x92`） | 全10byteを`0x20`（半角スペース）で埋める |
...
- **Responseキャラクタリスティック**: Peripheralが「通信開始応答」（送信種別`0x92`、通信種別`0x01`）と「データ受信応答」（送信種別`0x92`、通信種別`0x02`、6.6節「検出時送信データ」）の18byteペイロードをNotifyで通知する。
...
**Central役の切断完了について**: Step 1の`cancelPeripheralConnection`はCore Bluetoothへの切断要求であり、呼んだ時点では実際の切断は完了していない（実際の完了はCore Bluetoothから後で非同期に通知される）。`BluetoothManager`の画面向け完了通知（`completion`）自体はこれまで通り即座に返すが、内部的には実際の切断完了（または安全用の5秒タイムアウト）まで`BluetoothManager.isDisconnecting`を`true`に保持する。この間に新しい通信開始操作が行われた場合、直前の切断が完全に片付く前に新しいセッションを作ることになり実機の通信が不安定になる可能性があるため、Bluetooth接続画面側で操作を止める（SS設計書4.3.1参照）。Peripheral役の切断には対応するCore Bluetooth側の完了通知が無いため、この対象外（既知の未対応事項）。
...
| タイムアウト | 送信の瞬間から10秒。10秒以内に応答（6.6節「検出時送信データ」）を検出できなければ異常終了とする |
| 待受内容 | 送信種別`0x92`固定、通信種別`0x02`、送信元ID=送信時の送信先ID、送信先ID=送信時の送信元ID、入力データ=`0x20`埋め |
| 正常系 | 10秒以内に上記の応答を検出 → コール元に正常終了を通知 |
| 異常系 | 送信自体の失敗、または10秒以内に応答を検出できず → 切断処理（6.4節）を実行後、コール元に異常終了を通知 |
...
sequenceDiagram
    ...
    loop 受信終了要求まで継続
        ...
        alt チェックOK(送信元ID=接続先ID and 送信先ID=自端末ID and\n入力データ="YAMA" or "KAWA"(0x20埋め))
            BM->>C: Notify: 送信種別0x92,通信種別0x02,\n送信元ID=自端末ID,送信先ID=受信データの送信元ID,\n入力データ=0x20埋め
            BM-->>UI: 受信検出あり(受信データ)
        else チェックNG
            BM-->>UI: 受信検出なし
        end
...
| チェックOK時 | コール元に「受信検出あり」を通知（受信データを含む）。あわせて、受信したデータの送信元へ応答（検出時送信データ、下記）を1回送信する |
| 検出時送信データ | 送信種別`0x92`固定、通信種別`0x02`、送信元ID=自端末ID、送信先ID=受信データの送信元ID、入力データ=`0x20`埋め |
| チェックNG時 | コール元に「受信検出なし」を通知（応答は送信しない） |
...
| 有効時の挙動 | `BluetoothManager`の`startConnecting`／`startListening`／`sendData`が実際のCore Bluetooth通信を行わず、2秒の擬似的な待ち時間の後に成功したものとして扱う。`startListening`側はデータ受信処理（`DataReceiver`）を生成しないため、データ受信画面は「受信中」が画面内表示されたまま実際の受信処理は行われない（画面遷移の確認のみ） |

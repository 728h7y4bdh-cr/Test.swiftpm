...
| 1.3 | 2026-08-23 | 4.3.2節・4.3.3節「接続中」ダイアログに「キャンセル」ボタンを追加し、「キャンセル中」の状態・4.4節ダイアログ文言一覧（No.8）を追記。6.2節・6.5節「受信中」をダイアログから画面内表示に変更（「戻る」ボタン操作を妨げないため） |
| 1.4 | 2026-08-23 | 4.4節ダイアログ文言一覧の「キャンセル中」を、時系列に沿うようNo.8からNo.6（「接続成功」の直後）へ移動 |
| 1.5 | 2026-08-23 | 3章「表示内容」を「Sample App」から「Sample App for BlueCom」に変更 |
| 1.6 | 2026-09-06 | 3章「表示内容」を「Sample App for BlueCom」から「BlueCom」に変更 |
...
flowchart TD
    ...
    G -->|異常終了| J[ダイアログ表示:\n「該当する端末がありませんでした」]
    J --> Z2[画面遷移なし]
    E -->|キャンセルタップ| K[ダイアログ表示:「キャンセル中」]
    K --> L[Bluetooth通信開始処理を中断]
    L --> Z3[画面遷移なし]
```

| 状態 | ダイアログ表示 |
|---|---|
| 処理中 | 「接続中」を、「キャンセル」ボタン付きで表示する（PS設計書 6.2節のBluetooth通信開始処理を実行） |
| キャンセル | 「キャンセル」タップ時、「キャンセル中」（ボタンなし）に切り替え、Bluetooth通信開始処理を中断する。中断（Bluetooth通信切断処理、PS設計書6.4節）が完了次第、ダイアログを閉じる。画面遷移は行わない |
...
| 状態 | ダイアログ表示 |
|---|---|
| 処理中 | 「接続中」を、「キャンセル」ボタン付きで表示する（PS設計書 6.3節のBluetooth待受開始処理を実行） |
| キャンセル | 「キャンセル」タップ時、「キャンセル中」（ボタンなし）に切り替え、Bluetooth待受開始処理を中断する。中断（Bluetooth通信切断処理、PS設計書6.4節）が完了次第、ダイアログを閉じる。画面遷移は行わない |
...
| 4 | 接続中 | 通信開始/待受処理の実行中 | キャンセル |
| 5 | 接続成功 | 通信開始/待受処理の正常終了時 | なし（1秒後に自動で閉じて画面遷移） |
| 6 | キャンセル中 | 「接続中」で「キャンセル」タップ時 | なし（処理の中断完了で自動的に閉じる） |
| 7 | 該当する端末がありませんでした | 通信開始/待受処理の異常終了時 | OK |
| 8 | しばらくしてから操作してください | 直前の切断処理未完了時 | OK |
...
| No | 状態 | 内容 |
|---|---|---|
| 1 | 受信中 | 「受信中」を画面内に固定表示する（ダイアログにはしない。表示中も「戻る」ボタンの操作は可能） |
...
| No | 文言 | 表示契機 | ボタン | 備考 |
|---|---|---|---|---|
| 1 | 受信中 | 画面遷移直後、受信処理開始時（表示固定） | なし | ダイアログではなく画面内表示（6.2節参照）。表示中も「戻る」ボタンの操作は可能 |

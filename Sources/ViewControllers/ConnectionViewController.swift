...
/// Bluetooth接続画面（SS設計書「Bluetooth接続画面仕様」 / UI設計書「Bluetooth接続画面」）。
/// 要件定義書「Bluetooth接続画面」に対応する画面。
final class ConnectionViewController: CommunicationBaseViewController {
    // UI設計書「Bluetooth接続画面」内「部品一覧」：自端末ID／接続先IDの入力欄、接続開始／待受開始ボタン
    ...
    /// 入力欄・ボタンをまとめたコンテナ。Bluetooth使用許可が下りるまで非表示にする（SS設計書「Bluetooth使用許可確認」）
    private let inputContainer = UIStackView()

    private let authorizationChecker = BluetoothAuthorizationChecker()
    /// SS設計書「Bluetooth使用許可確認」の「不許可」ダイアログを二重に表示しないための状態フラグ
    ...
    override func viewDidLoad() {
        ...
        // 許可確認が完了するまでは入力項目・ボタンを隠しておく（SS設計書「Bluetooth使用許可確認」の「許可」時に表示する）
        inputContainer.isHidden = true

        #if DEBUG
        // デバッグ機能：TOP画面からBluetooth接続画面へ遷移した最初のタイミングで、
        // プレビュー確認用バイパスの状態確認・監視を開始する（PS設計書「プレビュー確認用バイパス」）。
    ...
    /// SS設計書「Bluetooth接続画面仕様」内「入力項目」の「画面遷移時、内部パラメータ『自端末ID』『接続先ID』の値をそれぞれの入力項目に表示する」
    ...
    // MARK: - UI構築（UI設計書「Bluetooth接続画面」内「ワイヤーフレーム」「部品一覧」）
    ...
    private func setUpInputUI() {
        ...
        // SS設計書「ボタン仕様」の「接続開始ボタンと待受開始ボタンは横並びで表示」
    ...
    /// SS設計書「Bluetooth接続画面仕様」内「入力項目」の「入力可能文字は半角数値のみ」に合わせ、数値専用キーボードを指定する
    ...
    // MARK: - Bluetooth使用許可確認（SS設計書「Bluetooth使用許可確認」）

    /// SS設計書「Bluetooth使用許可確認」の一連の流れを実行する。
    ...
    private func checkBluetoothAuthorization() {
        authorizationChecker.check { [weak self] granted in
            ...
            // SS設計書「Bluetooth使用許可確認」の「『許可』を選択した場合：ダイアログの表示を消し、入力項目・ボタンを表示する」
            self.inputContainer.isHidden = !granted
            if granted {
                self.isPermissionDeniedAlertPresented = false
            } else {
                // SS設計書「Bluetooth使用許可確認」の「『不許可』を選択した場合：固定文言のダイアログを表示する」
                self.presentPermissionDeniedAlertIfNeeded()
            }
        }
    }

    /// SS設計書「Bluetooth使用許可確認」「ダイアログ文言一覧（Bluetooth接続画面）」の「設定からBluetoothの使用を許可してアプリを再起動してください。」
    ...
    // MARK: - ボタン処理（SS設計書「ボタン仕様」）

    /// 「接続開始」ボタン（SS設計書「『接続開始』ボタン」）：不一致チェック→Bluetooth通信開始処理（PS設計書「Bluetooth通信開始処理（Central側）」）を実行し、
    /// 正常終了ならデータ送信画面へ遷移する。
    ...
    /// 「待受開始」ボタン（SS設計書「『待受開始』ボタン」）：不一致チェック→Bluetooth待受開始処理（PS設計書「Bluetooth待受開始処理（Peripheral側）」）を実行し、
    /// 正常終了ならデータ受信画面へ遷移する。
    ...
    /// 「接続開始」「待受開始」共通の処理フロー。
    /// SS設計書「共通処理（不一致チェック）」〜「『接続開始』ボタン」／「『待受開始』ボタン」の「処理中ダイアログ表示→結果分岐」までを
    ...
    private func beginHandshake(
        ...
        startOperation(myID, targetID) { [weak self] success in
            ...
            processingAlert.dismiss(animated: true) {
                if success {
                    // SS設計書「『接続開始』ボタン」／「『待受開始』ボタン」の「正常終了の場合、『接続成功』を1秒間表示した後、対応する画面へ遷移する」
                    self.presentConnectionSucceededAlert(then: onSuccess)
                } else {
                    // SS設計書「『接続開始』ボタン」／「『待受開始』ボタン」の「異常終了の場合、『該当する端末がありませんでした』を表示し、画面遷移は行わない」
    ...
    /// 「接続中」ダイアログの「キャンセル」ボタン仕様（SS設計書「『接続開始』ボタン」／「『待受開始』ボタン」）。
    ...
    /// PS設計書「ID変換仕様」が3桁固定のIDを前提としているため、
    ...
    /// SS設計書「『接続開始』ボタン」／「『待受開始』ボタン」の「正常終了：『接続成功』をボタンなしで1秒間表示した後、
    /// 自動的にダイアログを閉じて画面遷移する」。
...
// MARK: - UITextFieldDelegate（SS設計書「Bluetooth接続画面仕様」内「入力項目」のバリデーション）

extension ConnectionViewController: UITextFieldDelegate {
    /// SS設計書「Bluetooth接続画面仕様」内「入力項目」の「入力可能文字は半角数値のみ」「入力桁数は最大3桁までとする」を満たすよう、

...
/// TOP画面（SS設計書「TOP画面仕様」 / UI設計書「TOP画面」）。
/// 要件定義書「TOP画面」：アイコンタップ時に最初に表示する起動画面。
final class TopViewController: UIViewController {
    ...
    /// SS設計書「TOP画面仕様」：画面表示から5秒経過後、自動的にBluetooth接続画面へ遷移する。
    ...
    /// SS設計書「TOP画面仕様」：画面中央に文字列「BlueCom」を表示する
    /// （要件定義書「TOP画面」：「画面中央に『BlueCom』の文字を表示する」）。
    /// UI設計書「TOP画面」内「部品一覧」の「lblTitle」参照：黒文字・1行・通常書体（太字にしない）。
    ...
    /// スタックの置き換え（setViewControllers）で遷移する（UI設計書「デザイン方針」の「ナビゲーション」方針）。

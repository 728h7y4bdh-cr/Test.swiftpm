...
/// データ受信画面（SS設計書「データ受信画面仕様」 / UI設計書「データ受信画面」）。
/// 要件定義書「データ受信画面」に対応する画面。Bluetooth接続画面の「待受開始」成功後（Peripheral役）に遷移してくる。
final class ReceiveViewController: CommunicationBaseViewController {
    // UI設計書「データ受信画面」内「部品一覧」：受信データの小項目3つ（送信元ID／送信先ID／データ）と「受信再開」ボタン
    ...
    /// 「受信中」の状態表示。SS設計書「ダイアログ文言一覧（データ受信画面）」の「受信中」はダイアログではなく画面内表示とする
    /// （ダイアログにすると背面の「戻る」ボタン操作が塞がれてしまうため）。
    private let statusLabel = UILabel()

    /// SS設計書「ダイアログ文言一覧（データ受信画面）」の「OKボタンが操作されないまま5秒経過した場合、ダイアログの表示を『受信中』に戻す」用のタイマー
    ...
    override func viewDidLoad() {
        ...
        // UI設計書「データ受信画面」内「部品一覧」：「戻る」はナビゲーションバー左のUIBarButtonItemとして配置する
        navigationItem.leftBarButtonItem = UIBarButtonItem(
            title: "戻る", style: .plain, target: self, action: #selector(didTapBack)
        )
        setUpUI()
        // データ受信処理（PS設計書「データ受信処理（Peripheral側／データ受信画面から呼び出し）」）の結果通知（受信検出あり／なし）を本画面で受け取る
        BluetoothManager.shared.receiveDelegate = self
        // Bluetooth接続の予期しない切断通知を受け取る（SS設計書「データ受信画面仕様」内「予期しない切断時の仕様」）
    ...
    /// SS設計書「画面遷移直後の処理フロー」の起点：部品表示後、受信処理を開始する。
    ...
    // MARK: - UI構築（UI設計書「データ受信画面」内「ワイヤーフレーム」「部品一覧」）

    private func setUpUI() {
        // SS設計書「画面遷移時の表示」の大項目『受信データ』
        ...
        // SS設計書「画面遷移時の表示」の小項目『送信元ID』『送信先ID』『データ』：画面遷移時は全て空欄表示
        ...
        // 「受信再開」ボタンは受信検出あり（SS設計書「画面遷移直後の処理フロー」）が起きるまで非表示
    ...
    // MARK: - 受信処理の開始（SS設計書「画面遷移直後の処理フロー」）

    /// SS設計書「画面遷移直後の処理フロー」の「『受信中』を画面内に表示後、データ受信処理を開始する」。
    /// 画面遷移直後（viewDidAppear）と、「受信再開」ボタン押下時（didTapResume）の
    /// 2箇所から呼ばれる共通処理。
    private func beginReceiving() {
        showReceivingStatus()
        BluetoothManager.shared.startReceiving() // PS設計書「データ受信処理（Peripheral側／データ受信画面から呼び出し）」開始
    }

    /// SS設計書「ダイアログ文言一覧（データ受信画面）」の「受信中：画面内に固定表示する（表示固定。ダイアログにはしない）」。
    ...
    /// SS設計書「『受信再開』ボタン仕様：タップするとデータ受信処理を再度開始する」
    @objc private func didTapResume() {
        resumeButton.isHidden = true
        beginReceiving()
    }

    // MARK: - 「戻る」ボタン（SS設計書「データ受信画面仕様」内「『戻る』ボタン仕様」）

    /// SS設計書「データ受信画面仕様」内「『戻る』ボタン仕様」の3ステップをそのまま実行する：
    ///   1. データ受信処理に「受信終了要求」を通知し、「受信終了完了」の通知を受けるまで待機する
    ///   2. Bluetooth通信切断処理（PS設計書「Bluetooth通信切断処理」）を実行する
    ...
    @objc private func didTapBack() {
        ...
        let finishAndPop: () -> Void = { [weak self] in
            ...
            // Step 2: Bluetooth通信切断処理（PS設計書「Bluetooth通信切断処理」）
    ...
    /// 「データ受信完了」ダイアログ（SS設計書「ダイアログ文言一覧（データ受信画面）」、OKボタンあり）の表示に使用する。
    ...
    // MARK: - CommunicationBaseViewController（SS設計書「データ受信画面仕様」内「予期しない切断時の仕様」）
...
// MARK: - BluetoothManagerReceiveDelegate（PS設計書「データ受信処理（Peripheral側／データ受信画面から呼び出し）」の結果通知）

extension ReceiveViewController: BluetoothManagerReceiveDelegate {
    /// 受信検出あり（PS設計書「データ受信処理（Peripheral側／データ受信画面から呼び出し）」の「チェックOK時：コール元に『受信検出あり』を通知」）。
    /// SS設計書「画面遷移直後の処理フロー」の一連の流れを実行する：
    ///   データ受信処理に「受信終了要求」を通知→「受信終了完了」を受けた後、
    ///   小項目（送信元ID／送信先ID／データ）に受信データを表示し「受信再開」ボタンを表示、
    ///   その状態のまま「データ受信完了」をダイアログ表示する（OKボタンあり。SS設計書「ダイアログ文言一覧（データ受信画面）」）。
    func bluetoothManager(_ manager: BluetoothManager, didDetect payload: Payload) {
        invalidDataRevertTimer?.invalidate()
        invalidDataRevertTimer = nil
        currentInvalidDataAlert = nil

        // 「受信終了要求」通知〜「受信終了完了」待機（PS設計書「データ受信処理（Peripheral側／データ受信画面から呼び出し）」）
        BluetoothManager.shared.stopReceiving { [weak self] in
            guard let self else { return }
            // 背面の表示を先に更新してから、SS設計書「ダイアログ文言一覧（データ受信画面）」の「表示された時点で背面は
    ...
    /// 受信検出なし（PS設計書「データ受信処理（Peripheral側／データ受信画面から呼び出し）」の「チェックNG時：コール元に『受信検出なし』を通知」）。
    /// SS設計書「画面遷移直後の処理フロー」「ダイアログ文言一覧（データ受信画面）」に対応：
    ...
    func bluetoothManagerDidFailToDetect(_ manager: BluetoothManager) {
        let showInvalidDataAlert: () -> Void = { [weak self] in
            ...
            // OKタップ時：SS設計書「ダイアログ文言一覧（データ受信画面）」の「OKボタン操作時は受信中表示に戻す」
            ...
            // SS設計書「ダイアログ文言一覧（データ受信画面）」の「OKボタンが操作されないまま5秒経過した場合、受信中表示に戻す」
            self.invalidDataRevertTimer?.invalidate()

...
/// 状態遷移の契機（PS設計書「状態遷移契機一覧」の10行に1:1で対応するイベント）。
///
/// 状態遷移を必ずこのイベント経由で行うことで、「どの契機でどの状態になるか」という
/// ルールをStatusManager 1箇所に集約する（状態遷移の一元管理）。呼び出し元（各処理クラス）は
/// 遷移先の`AppStatus`を直接指定せず、PS設計書「状態遷移契機一覧」の行に対応するイベント名を渡すだけでよい。
enum AppStatusTransitionEvent {
    /// アプリ起動時
    case appLaunched
    /// Bluetooth通信切断処理完了後
    case disconnectCompleted
    /// Bluetooth通信開始処理にて、通信開始のデータ送信時
    case connectionStartRequestSent
    /// Bluetooth通信開始処理にて、コール元に正常終了を通知する時
    case connectionStartSucceeded
    /// データ送信処理にて、データ送信完了時
    case dataSendCompleted
    /// Bluetooth通信待受処理にて、60秒間のデータ待受開始時
    case listenStarted
    /// Bluetooth通信待受処理にて、コール元に正常終了を通知する時
    case listenSucceeded
    /// データ受信画面にて、「再開」ボタンをタップしてデータ受信処理を開始した時
    /// （待受成功直後にデータ受信処理を開始する初回のタイミングも、同じ遷移先のためこのイベントを用いる）
    case receivingStarted
    /// データ受信処理が「受信終了要求」を検出して、コール元に「受信終了完了」を通知する時
    case receiveStopRequested
    /// データ送信処理にて、データ送信開始時
    case dataSendStarted
}

/// 通信状態（ステータス）の保持・遷移を一元管理するクラス（PS設計書「ステータス（状態）仕様」）。
///
/// 状態遷移のルールはPS設計書「状態遷移契機一覧」の対応表をそのまま`transitionTable`として

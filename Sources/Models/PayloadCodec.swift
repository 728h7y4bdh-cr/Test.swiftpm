...
/// ペイロードのエンコード／デコード（PS設計書「通信フォーマット仕様」）。
/// `Payload`構造体と、実際にBluetoothで送受信する18byteの`Data`とを相互変換する。
enum PayloadCodec {

    /// Payload → 18byteのバイナリに変換する（PS設計書「ペイロード構成（全18byte固定長）」の並び順でバイト列を組み立てる）。
    ...
    static func encode(_ payload: Payload) -> Data {
        ...
        // PS設計書「ID変換仕様」：3桁の数値文字列をASCII(0x30〜0x39)にエンコードし3byteにする
        ...
        // 入力データは呼び出し側で既に用途別（PS設計書「入力データ領域（10byte）の用途別内容」）に整形済みの10byteを渡す想定だが、
        // 念のためここでも10byteに揃える
        data.append(fixedLength(payload.inputData, length: Payload.inputDataLength))
        return data
    }

    /// 18byteのバイナリ → Payload に変換する（PS設計書「ペイロード構成（全18byte固定長）」のオフセット定義に基づき分解する）。
    /// 長さが18byteでない、または送信種別／通信種別が規定のコード（PS設計書「送信種別コード」「通信種別コード」）にない場合はフォーマット不正としてnilを返す。
    static func decode(_ data: Data) -> Payload? {
        guard data.count == Payload.totalLength else { return nil }
        let bytes = [UInt8](data)

        // PS設計書「送信種別コード」：0x29(要求) / 0x92(応答) 以外は不正データ
        guard let type = PayloadType(rawValue: bytes[0]) else { return nil }
        // PS設計書「通信種別コード」：0x01(通信開始・待受) / 0x02(データ送受信) 以外は不正データ
        guard let commType = CommunicationType(rawValue: bytes[1]) else { return nil }

        // オフセット 2〜4:送信元ID(3byte)、5〜7:送信先ID(3byte)、8〜17:入力データ(10byte)（PS設計書「ペイロード構成（全18byte固定長）」）
    ...
    /// PS設計書「入力データ領域（10byte）の用途別内容」：通信開始要求／応答（通信種別0x01）で使用する
    /// （要件定義書「通信仕様」：「入力データ（10byte）：0x20で埋める」に対応）。
    static var blankInputData: Data {
        Data(repeating: 0x20, count: Payload.inputDataLength)
    }

    /// 文字列をASCII変換し、10byteになるよう残りを0x20で埋めたデータ。
    /// PS設計書「入力データ領域（10byte）の用途別内容」：データ送信（通信種別0x02）で、画面で選択された"YAMA"／"KAWA"を変換する際に使用する
    /// （要件定義書「データ送信処理」：「空きバイトは0x20埋め」に対応）。

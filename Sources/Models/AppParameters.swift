...
/// 内部パラメータ「自端末ID」「接続先ID」を保持するクラス（PS設計書「内部パラメータ」）。
...
final class AppParameters {
    ...
    /// 自端末ID（3桁の数値文字列 "000"〜"999"）。初期値は"000"（PS設計書「内部パラメータ」）
    private(set) var myID: String = "000"
    /// 接続先ID（3桁の数値文字列 "000"〜"999"）。初期値は"001"（PS設計書「内部パラメータ」）
    private(set) var targetID: String = "001"

    /// 初期化処理（PS設計書「初期化処理」）：アプリ起動時に1度だけ呼び出す。
    /// 自端末IDに"000"、接続先IDに"001"をセットする
    /// （要件定義書「初期化処理」：「自端末ID」に000、「接続先ID」に001をセットする、に対応）。
    ...
    /// SS設計書「共通処理（不一致チェック）」の「不一致」時の処理に対応する。
